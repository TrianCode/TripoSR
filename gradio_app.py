...
import argparse  # <<< MODIFIKASI: Tambahkan import argparse di atas
import gradio as gr
import numpy as np
import rembg
import torch
...
# --- MODIFIKASI: Pindahkan definisi parser ke atas agar bisa dibaca sebelum launch ---
parser = argparse.ArgumentParser()
parser.add_argument(
    '--pretrained-model-path', 
    type=str, 
    default=None, 
    help='Path to a custom pretrained model (.ckpt)'
)
parser.add_argument('--username', type=str, default=None, help='Username for authentication')
parser.add_argument('--password', type=str, default=None, help='Password for authentication')
parser.add_argument('--port', type=int, default=7860, help='Port to run the server listener on')
...
# Parse a early version of args to get the model path
# We only care about the model path here, the rest will be parsed again later
# This is a common way to handle arguments that affect pre-launch setup
...
# <<< MODIFIKASI DIMULAI DI SINI: Logika Pemuatan Model Dinamis >>>
if args.pretrained_model_path:
    print(f"Loading custom model from: {args.pretrained_model_path}")
    # Jika menggunakan model kustom, kita hanya perlu path ke file .ckpt
    # dan file config.yaml harus ada atau kita tentukan secara default.
    # Asumsikan config.yaml default bisa digunakan.
    model = TSR.from_pretrained(
        args.pretrained_model_path,
        config_name="config.yaml",  # Asumsi menggunakan config standar
    )
else:
    print("Loading default model from stabilityai/TripoSR...")
    model = TSR.from_pretrained(
        "stabilityai/TripoSR",
        config_name="config.yaml",
        weight_name="model.ckpt",
...
def create_metrics_radar_chart(current_metrics):
    # ... (sisa kode fungsi ini tidak perlu diubah)
    """Create a radar chart comparing the current metrics with historical averages"""
    # Define metrics to show (lower is better for UHD, TMD, CD; higher is better for IoU and F1)
    metrics_to_show = {
        'f1_score': {'display': 'F1', 'invert': False},
...
with gr.Blocks(title="3D Model Generation") as interface:
    # ... (sisa kode Gradio UI tidak perlu diubah)
    gr.Markdown(
        """    
# Generasi Model 3D dari Gambar
...
if __name__ == '__main__':
    # <<< MODIFIKASI: Gunakan args yang sudah diparsing di atas >>>
    # Parser sudah didefinisikan dan diparsing sebagian di atas
    # Sekarang kita parse lagi untuk mendapatkan semua argumen untuk launch
    final_args = parser.parse_args()
    
    interface.queue(max_size=final_args.queuesize)
    
    auth = None
    if final_args.username and final_args.password:
        auth = (final_args.username, final_args.password)
    
    try:
        interface.launch(
            server_port=final_args.port,
            server_name="0.0.0.0" if final_args.listen else None,
            share=final_args.share,
            auth=auth,
            debug=True
        )
    except Exception as e:
